...
export default function StudentCard({ student, onStatusChange }) {
  ...
  const handlePause = () => {
    showToast(`Session paused for ${student.roll}`, 'warning');
    onStatusChange?.(student.roll, 'paused');
  };
  const handleResume = () => {
    showToast(`Session resumed for ${student.roll}`, 'success');
    onStatusChange?.(student.roll, 'active');
  };
  const handleStop = () => {
    if (window.confirm(`Stop session for ${student.roll}?`)) {
      showToast(`Session stopped for ${student.roll}`, 'info');
      onStatusChange?.(student.roll, 'completed');
    }
  };
